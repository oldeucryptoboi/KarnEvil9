        ...
        <table className="w-full text-sm">
          <thead className="bg-[var(--card)]">
            <tr>
              ...
              <th className="text-left p-3 font-medium text-[var(--muted)]">Task</th>
              <th className="text-left p-3 font-medium text-[var(--muted)]">Schedule</th>
          ...
          <tbody className="divide-y divide-[var(--border)]">
            {schedules.map((s) => (
              <tr key={s.id} className="hover:bg-white/[0.02]">
                ...
                <td className="p-3 text-sm truncate max-w-xs">{s.task_text}</td>
                <td className="p-3 font-mono text-xs text-[var(--muted)]">
                  {s.cron ?? (s.interval_ms ? `every ${s.interval_ms / 1000}s` : "-")}
                </td>
                <td className="p-3">
                  <span className={`inline-block rounded-full px-2 py-0.5 text-xs ${s.enabled ? "bg-green-500/20 text-green-400" : "bg-gray-500/20 text-gray-400"}`}>
