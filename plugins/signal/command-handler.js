/**
 * CommandHandler — handles text commands sent via Signal DMs.
 * Commands: status, cancel [id], help
 */
export class CommandHandler {
  /**
   * @param {object} opts
   * @param {import("./signal-client.js").SignalClient} opts.signalClient
   * @param {import("./session-bridge.js").SessionBridge} opts.sessionBridge
   * @param {object} [opts.logger]
   */
  constructor({ signalClient, sessionBridge, logger }) {
  ...
  async _handleStatus(sender) {
    ...
    if (sessions.length === 0) {
      await this.signalClient.sendMessage({ recipient: sender, message: "No active sessions." });
      return;
    }
    ...
    await this.signalClient.sendMessage({
      recipient: sender,
  ...
  async _handleCancel(sender, sessionId) {
    if (!sessionId) {
      ...
      if (!activeSessionId) {
        await this.signalClient.sendMessage({ recipient: sender, message: "No active session to cancel." });
        return;
      }
    ...
    if (!info) {
      await this.signalClient.sendMessage({ recipient: sender, message: `\u274C Session ${sessionId} not found.` });
      return;
    }
    ...
    await this.signalClient.sendMessage({
      recipient: sender,
      message: `\u{1F6D1} Session ${sessionId} cancel requested.`,
    });
  }
