/**
 * JournalRelay — subscribes to journal events, sends progress as Signal messages.
 */
import { formatJournalEvent } from "./message-formatter.js";

/** Events worth relaying to Signal */
...
export class JournalRelay {
   ...
   */
  constructor({ journal, signalClient, sessionBridge, approvalHandler, logger }) {
  ...
  async _handleEvent(event) {
    ...
    if (!sender) return; // Not a Signal-originated session
    ...
    await this.signalClient.sendMessage({ recipient: sender, message: formatted });
