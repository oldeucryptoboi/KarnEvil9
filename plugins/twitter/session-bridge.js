/**
 * SessionBridge — maps Twitter sender IDs to KarnEvil9 sessions.
 * One active session per sender at a time.
 */
export class SessionBridge {
  ...
  constructor({ sessionFactory, maxConcurrentSessions = 10, sessionTimeout = 300000 }) {
    ...
    /** @type {Map<string, string>} senderId -> sessionId */
    this.senderToSession = new Map();
  }

  /**
   * Check if a sender already has an active session.
   * @param {string} sender - Twitter user ID
   * @returns {boolean}
   */
  ...
  /**
   * Create a new KarnEvil9 session from a Twitter DM.
   * @param {object} opts
   * @param {string} opts.taskText - cleaned task text
   * @param {string} opts.sender - sender Twitter user ID
  ...
  async createSession({ taskText, sender }) {
    ...
    const task = {
      task_id: `twitter-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
      text: taskText,
  ...
  /**
   * Get sender ID for a session ID.
   * @param {string} sessionId
   * @returns {string | undefined}
   */
