/**
 * SessionBridge — maps WhatsApp sender JIDs to KarnEvil9 sessions.
...
export class SessionBridge {
  ...
  constructor({ sessionFactory, maxConcurrentSessions = 10, sessionTimeout = 300000 }) {
    ...
    /** @type {Map<string, string>} sender JID -> sessionId */
  ...
  /**
   * Create a new KarnEvil9 session from a WhatsApp message.
   * @param {object} opts
   * @param {string} opts.taskText - cleaned task text
   * @param {string} opts.sender - sender WhatsApp JID
  ...
  async createSession({ taskText, sender }) {
    ...
    const task = {
      task_id: `whatsapp-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
      text: taskText,
  ...
  /**
   * Get sender JID for a session ID.
   * @param {string} sessionId
   * @returns {string | undefined}
   */
